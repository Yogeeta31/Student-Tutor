...
module.exports.approveNewContent = (req, res) => {
  ...
  if (isApproved == 1) {
          ...
          dbConnection.query(list, async (err, result) => {
            ...
            if (contentType == "image") {
              ...
              dbConnection.query(approve, async (err, result) => {
                ...
                res
                  .status(200)
                  .json({ message: "Image Has been Approved & Updated" });
              });
            }
            if (contentType == "bio") {
              const approve = `UPDATE USER
    ...
    dbConnection.query(approve, async (err, result) => {
      ...
      res
        .status(200)
        .json({ message: "Content has been Rejected by the moderator" });
    });
  }
};
...
module.exports.listOfTutorsWithNewContent = (req, res) => {
  const approve = `SELECT APPROVAL.*,USER.EMAIL FROM APPROVAL,TUTOR,USER WHERE APPROVAL.IS_APPROVED = 0 and APPROVAL.TUTOR_ID = TUTOR.TUTOR_ID and USER.USER_ID = TUTOR.USER_ID`;

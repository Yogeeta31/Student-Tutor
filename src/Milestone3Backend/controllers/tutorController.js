...
module.exports.getTutorDetails = async (req, res) => {
  ...
  tutor["reviews"] = [];
  let sqlReviewQuery = `SELECT r.REVIEW, r.RATING FROM TUTOR t  INNER JOIN REVIEWS r  ON (t.USER_ID = r.TO_USER_ID) WHERE t.USER_ID = ${user_id} ORDER BY r.RATING DESC LIMIT 3`;
  let reviews = null;
  try {

...
const createTutors = async () => {
  ...
  var sqlCreateUserTable = `CREATE TABLE USER (USER_ID INT NOT NULL AUTO_INCREMENT, NAME VARCHAR(90), GENDER VARCHAR(10), MOBILE_NO BIGINT, EMAIL VARCHAR(45), PASSWORD VARCHAR(100), ROLE_ID INT, BIO VARCHAR(150), REGISTERED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP, LAST_LOGIN TIMESTAMP DEFAULT CURRENT_TIMESTAMP, HAS_PERMISSION TINYINT DEFAULT 1 NOT NULL, ROLES_ROLE_ID INT, REVIEW_ID INT, IMAGE VARCHAR(255), PRIMARY KEY (USER_ID), UNIQUE(EMAIL))`;
